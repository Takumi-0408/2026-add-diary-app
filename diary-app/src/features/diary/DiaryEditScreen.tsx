...
import { fetchDiaryById, updateDiary, uploadImage } from '../../services/diaryService';
...
export function DiaryEditScreen() {
  ...
  useEffect(() => {
    fetchDiaryById(diaryId).then((diary) => {
      setIcon(diary.icon);
      setTitle(diary.title);
      setBody(diary.body);
      setDate(diary.date);
      setExistingImageUrl(diary.imageUrl);
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (imageUri) {
        ...
        if (user) imageUrl = await uploadImage(user.uid, imageUri);
      }
      await updateDiary(diaryId, { icon, title, body, date, imageUrl });
      navigation.goBack();
    } catch (e: any) {
      ...
      <View style={styles.form}>
        ...
        {(imageUri || existingImageUrl) && (
          <View>
            ...
            <TouchableOpacity onPress={() => { setImageUri(undefined); setExistingImageUrl(undefined); }}>
